...
export default function RelationshipBreakerPage() {
  ...
  const currentQuestion = BREAKER_QUESTIONS[currentQ]
  const isLast = currentQ === BREAKER_QUESTIONS.length - 1
  ...
  async function submitDiagnosis() {
    ...
    try {
      const res = await fetch('/api/diagnose-breaker', {
        method: 'POST',
  ...
  function handleShare() {
    ...
    const text = `${typeInfo.emoji} 人間関係の壊し方タイプ診断：「${typeInfo.name}」\n\n${result.shareText}\n\n#人間関係の壊し方診断`
      ...
      <main className="min-h-screen flex flex-col items-center justify-center px-4">
        <div className="text-center space-y-6">
          <div className="relative w-20 h-20 mx-auto">
            <div className="absolute inset-0 rounded-full border-4 border-slate-200 animate-ping" />
            <div className="absolute inset-2 rounded-full border-4 border-slate-500 animate-spin border-t-transparent" />
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400">AIがあなたのパターンを解析しています</p>
          </div>
        </div>
      </main>
    )
        ...
        <div className="max-w-md w-full space-y-6">
          <div className={`rounded-3xl bg-gradient-to-br ${typeInfo.bgGradient} p-8 shadow-sm`}>
            <div className="text-center space-y-3 mb-6">
              ...
              <div>
                <p className="text-xs text-gray-500 mb-1">あなたの「人間関係の壊し方」タイプは</p>
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-5 space-y-4">
              <div>
                <p className="text-xs font-medium text-gray-500 mb-2 flex items-center gap-1">
                  <Sparkles className="w-3 h-3" /> AIによるパターン解剖
                </p>
                <p className="text-sm text-gray-700 leading-relaxed">{result.diagnosis}</p>
              </div>

              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs font-medium text-gray-500 mb-2">🎭 あなたの「壊し方」が言葉を持ったとしたら</p>
  ...
  const progress = (currentQ / BREAKER_QUESTIONS.length) * 100
      ...
      <div className="max-w-md w-full space-y-6">
          ...
          <div className="flex-1">
            <div className="flex justify-between items-center mb-1">
              <span className="text-xs text-gray-400">
                {currentQ + 1} / {BREAKER_QUESTIONS.length}
              </span>
              <span className="text-xs text-gray-400">
                人間関係の壊し方タイプ診断
              </span>
            ...
            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
              <div
                className="h-full bg-gradient-to-r from-slate-400 to-gray-500 rounded-full transition-all duration-500"
        ...
        <div className="bg-white rounded-3xl p-7 shadow-sm border border-gray-50 space-y-5">
          ...
          <textarea
            ...
            rows={4}
            className="w-full resize-none rounded-2xl bg-gray-50 border-0 p-4 text-sm text-gray-700 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-slate-200 leading-relaxed transition-all"
          />
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={handleNext}
            disabled={!canNext}
            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-slate-600 to-gray-700 text-white rounded-2xl py-4 text-sm font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:from-slate-700 hover:to-gray-800 transition-all shadow-sm"
          >
            {isLast ? (
              <>
