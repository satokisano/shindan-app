...
export default function BrainVoicePage() {
  ...
  function handleShare() {
    ...
    const text = `${typeInfo.emoji} 脳内の声タイプ診断結果：「${typeInfo.name}」\n\n${result.shareText}\n\n#脳内うるさい声診断`
      ...
      <main className="min-h-screen flex flex-col items-center justify-center px-4">
        <div className="text-center space-y-6">
          <div className="relative w-20 h-20 mx-auto">
            ...
            <div className="absolute inset-2 rounded-full border-4 border-purple-400 animate-spin border-t-transparent" />
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-gray-400">AIがあなたの言葉を本気で読んでいます</p>
          </div>
        </div>
      </main>
    )
      ...
      return (
        <main className="min-h-screen flex flex-col items-center justify-center px-4">
          <div className="text-center space-y-4">
            ...
            <button onClick={handleRetry} className="text-sm text-purple-500 underline">もう一度試す</button>
          </div>
        </main>
      )
    }
        ...
        <div className="max-w-md w-full space-y-6">
          <div className={`rounded-3xl bg-gradient-to-br ${typeInfo.bgGradient} p-8 shadow-sm`}>
            <div className="text-center space-y-3 mb-6">
              ...
              <div>
                <p className="text-xs text-gray-500 mb-1">あなたの脳内の声タイプは</p>
            ...
            <div className="bg-white/70 backdrop-blur-sm rounded-2xl p-5 space-y-4">
              <div>
                <p className="text-xs font-medium text-gray-500 mb-2 flex items-center gap-1">
                  <Sparkles className="w-3 h-3" /> AIによる分析
                </p>
                <p className="text-sm text-gray-700 leading-relaxed">{result.diagnosis}</p>
              </div>

              <div className="border-t border-gray-100 pt-4">
                <p className="text-xs font-medium text-gray-500 mb-2">🎙️ あなたの脳内の声が言いそうなこと</p>
      ...
      <div className="max-w-md w-full space-y-6">
        ...
        <div className="bg-white rounded-3xl p-7 shadow-sm border border-gray-50 space-y-5">
          ...
          <textarea
            ...
            rows={4}
            className="w-full resize-none rounded-2xl bg-gray-50 border-0 p-4 text-sm text-gray-700 placeholder:text-gray-300 focus:outline-none focus:ring-2 focus:ring-purple-200 leading-relaxed transition-all"
          />
        ...
        <div className="flex gap-3">
          ...
          <button
            onClick={handleNext}
            disabled={!canNext}
            className="flex-1 flex items-center justify-center gap-2 bg-gradient-to-r from-purple-500 to-indigo-500 text-white rounded-2xl py-4 text-sm font-medium disabled:opacity-30 disabled:cursor-not-allowed hover:from-purple-600 hover:to-indigo-600 transition-all shadow-sm"
          >
            {isLast ? (
              <>
